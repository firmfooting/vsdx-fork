...
class VisioFile:
    ...
    def _ensure_masters_for_shape(self, source_shape: 'Shape') -> str:
        ...
        master_name = (source_element.attrib.get('NameU')
                       or source_element.attrib.get('Name') or '')
        ...
        existing_numbers = [int(f[len(prefix):-4]) for f in self.zip_file_contents
        ...
        numeric_ids = [int(m.attrib['ID']) for m in self.masters_xml
                       if str(m.attrib.get('ID', '')).isdigit()]
        ...
        xml_to_file(ET.ElementTree(self.masters_xml),
                    f'{self._masters_folder}/masters.xml', self.zip_file_contents)
        ...
        rels_root.append(Element(f'{document_rels_namespace}Relationship',
                                 {'Id': new_rel_id,
        ...
        self._add_content_types_override(part_name_path=f'/visio/masters/{part_name}',
        ...
        new_master_page = Page(file_to_xml(final_part_path, self.zip_file_contents),
                               final_part_path,
                               master_name, str(new_id), new_rel_id, self)
